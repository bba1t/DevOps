...
testing()
{
    ...
    valgrind --trace-children=yes --track-fds=yes --track-origins=yes --leak-check=full --show-leak-kinds=all ./s21_grep $t &> test_s21_grep.log
    leak=$(grep -B100000 -A2 'at exit' test_s21_grep.log)
    (( COUNTER++ ))
    #if [[ $leak == *"0 errors from 0 contexts"* ]]
    if [[ $leak == *"0 bytes in 0 blocks"* ]]
    then
      (( SUCCESS++ ))
    ...
    leak=$(grep -B100000 -A2 'ERROR' test_s21_grep.log)
    (( COUNTER++ ))
    if [[ $leak == *"0 errors from 0 contexts"* ]]
    #if [[ $leak == *"0 bytes in 0 blocks"* ]]
    then
